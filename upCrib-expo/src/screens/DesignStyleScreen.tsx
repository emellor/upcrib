...
const DesignStyleScreen: React.FC<DesignStyleScreenProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Navigate to History screen first (will show placeholder soon)
      ...
      try {
        // STEP 1: Start the API call to get sessionId
        const createResult = await enhancedStyleRenovationApi.createRenovation(renovationRequest);
        const apiSessionId = createResult.data.sessionId;
        console.log('🆔 [DesignStyleScreen] Got API sessionId:', apiSessionId);
        
        // STEP 2: Create placeholder history item immediately
        const placeholderItem: DesignHistoryItem = {
          id: `${apiSessionId}-${Date.now()}`,
        ...
        console.log('� [DesignStyleScreen] Creating placeholder with API sessionId:', apiSessionId);
        await HistoryStorageService.saveDesignToHistory(placeholderItem);
        console.log('✅ [DesignStyleScreen] Placeholder created - user can now see generating item');
        
        // STEP 3: Start background polling (will update the placeholder when complete)
        console.log('🔔 Starting background polling for API session:', apiSessionId);
        await backgroundPollingService.addSession(apiSessionId);
        
        // STEP 4: Wait for completion (optional - background polling handles the update)
        try {
          await enhancedStyleRenovationApi.pollForCompletion(apiSessionId);
          console.log('✅ Enhanced style renovation completed successfully');
        } catch (pollError) {
          console.log('⚠️ Polling completed with error, but background service will handle it:', pollError);
        }
        
        // Show completion notification
        await notificationService.notifyGenerationComplete(apiSessionId);
