...
export class HistoryStorageService {
  ...
  static async saveDesignToHistory(design: DesignHistoryItem): Promise<void> {
    try {
      ...
      console.log(`💾 [HistoryStorage] All existing sessionIds:`, existingHistory.map(h => `${h.sessionId}(${h.status})`));
      
      if (existingIndex >= 0) {
        const oldItem = existingHistory[existingIndex];
        console.log(`💾 [HistoryStorage] UPDATING existing design: "${oldItem.title}" -> "${design.title}"`);
        console.log(`💾 [HistoryStorage] Status change: ${oldItem.status} -> ${design.status}`);
        console.log(`💾 [HistoryStorage] Keeping original createdAt: ${oldItem.createdAt}`);
        
        // Update existing design but preserve key original data
        existingHistory[existingIndex] = {
          ...designWithLocalPaths,
          id: oldItem.id, // Keep original ID
          createdAt: oldItem.createdAt, // Keep original creation time
          title: design.title || oldItem.title // Use new title if provided, otherwise keep original
        };
        
        console.log(`💾 [HistoryStorage] Update complete. New status: ${existingHistory[existingIndex].status}`);
      } else {
