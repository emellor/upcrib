...
class BackgroundPollingService {
  ...
  private async handleGenerationComplete(sessionId: string): Promise<void> {
    ...
    try {
      // Get the full renovation data
      const response = await apiClient.getEnhancedStyleRenovationStatus(sessionId);
      
      if (response.success && response.data && response.data.status === 'completed') {
        console.log(`💾 [BackgroundPolling] Updating history for completed renovation...`);
        
        // DIRECT HISTORY UPDATE - bypass the corrupted Enhanced Style API
        const { HistoryStorageService } = require('./historyStorage');
        
        // Find and update the existing generating item
        const existingDesigns = await HistoryStorageService.getDesignHistory();
        console.log(`🔍 [BackgroundPolling] Total designs in history: ${existingDesigns.length}`);
        console.log(`🔍 [BackgroundPolling] Looking for sessionId: ${sessionId}`);
        console.log(`🔍 [BackgroundPolling] Existing sessionIds:`, existingDesigns.map((d: any) => `${d.sessionId}(${d.status})`));
        ...
        if (existingDesign) {
          console.log(`💾 [BackgroundPolling] FOUND existing design: "${existingDesign.title}"`);
          
          // Update the existing design with completed data
          const updatedItem = {
            ...existingDesign,
            status: 'completed' as const,
            thumbnail: response.data.generatedImage ? `http://localhost:3001${response.data.generatedImage.url}` : existingDesign.thumbnail,
            originalImage: response.data.originalImage ? `http://localhost:3001${response.data.originalImage.url}` : existingDesign.originalImage,
            generatedImage: response.data.generatedImage ? `http://localhost:3001${response.data.generatedImage.url}` : undefined,
            metadata: {
              originalImageData: response.data.originalImage,
              generatedImageData: response.data.generatedImage,
              renovationData: response.data
            }
          };
          
          await HistoryStorageService.saveDesignToHistory(updatedItem);
          console.log(`✅ [BackgroundPolling] Successfully updated design to completed`);
        } else {
          console.log(`⚠️ [BackgroundPolling] No existing design found for session: ${sessionId}`);
          console.log(`📝 [BackgroundPolling] Creating new completed entry as fallback`);
          
          // Create a new completed entry since we can't find the generating one
          const newCompletedItem = {
            id: `${sessionId}-${Date.now()}`,
            sessionId: sessionId,
            createdAt: new Date().toISOString(),
            thumbnail: response.data.generatedImage ? `http://localhost:3001${response.data.generatedImage.url}` : '',
            originalImage: response.data.originalImage ? `http://localhost:3001${response.data.originalImage.url}` : '',
            generatedImage: response.data.generatedImage ? `http://localhost:3001${response.data.generatedImage.url}` : '',
            status: 'completed' as const,
            title: `Design ${sessionId.slice(-6)}`, // Generic title
            metadata: {
              originalImageData: response.data.originalImage,
              generatedImageData: response.data.generatedImage,
              renovationData: response.data
          ...
          await HistoryStorageService.saveDesignToHistory(newCompletedItem);
          console.log(`✅ [BackgroundPolling] Created new completed design entry`);
        }
      }
    } catch (error) {
      console.error(`❌ [BackgroundPolling] Failed to update completed renovation:`, error);
    }
